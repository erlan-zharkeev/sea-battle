...
export class Field {
  ...
  clearField() {
    ...
    const removeAttributes = ['deadZone', 'data-deadzone-id', 'data-class', 'data-name', 'data-direction']
    dots.forEach((dot) => {
      dot.remove()
    })
    cells.forEach((cell) => {
...
class Ship {
  ...
  _getAvailableCells() {
    ...
    const cells = field.querySelectorAll('.game__cell[data-status="empty"]')
    return cells
  }
...
function fillTable(table) {
  ...
  const shipArr = [4, 3, 3, 2, 2, 2, 1, 1, 1, 1]
  let id = 1
